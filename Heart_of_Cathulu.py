...
class Voxel(Entity):
	def __init__(self, position = (1,0,0), scale = (1,1,1)):
		...
		parent = scene,
		position = position,
		color = color.color(random.uniform(0,1),random.uniform(0,1), random.uniform(0,1)),
		scale = scale,
		collision = True,
		model = 'cube',
		texture = 'white_cube',
	...
	def update(self):
		...
		self.x += self.x*math.sin(time.time())/6 
		self.y += self.y*math.sin(time.time())/6 
		self.z += self.z*math.sin(time.time())/6 
