...
class Voxel(Entity):
	...
	def update(self):
		if abs(self.x) >5:
			self.x +=   (-math.cos(self.y/(abs(self.z**(0.5)) + 1))**(2))*time.dt*10
		else:
			self.x +=   (math.sin(self.y/(abs(self.z**(0.5)) + 1))**(2))*time.dt*10
		
		if abs(self.y) >5:
			self.z +=   (-math.cos(self.x/(abs(self.y**(0.5)) + 1))**(2))*time.dt*10
		else:
			self.z +=   (math.sin(self.x/(abs(self.y**(0.5)) + 1))**(2))*time.dt*10
		
		
		if abs(self.z) >5:
			self.y +=   (-math.cos(self.x/(abs(self.z**(0.5)) + 1))**(1))*time.dt*10 
		else:
			self.y +=   (math.sin(self.x/(abs(self.z**(0.5)) + 1))**(1))*time.dt*10 
